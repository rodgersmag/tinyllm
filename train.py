...
if __name__ == "__main__":
    ...
    if not os.path.exists("train.txt"):
        ...
        try:
            ...
            with open("train.txt", "w", encoding="utf-8") as f:
                f.write("\n".join(dataset["text"]))
            print("Dataset saved as train.txt")
    ...
    try:
        ...
        print("Error: train.txt not found. Dataset download may have failed.")
        exit(1)
    ...
    for epoch in range(config.num_epochs):
        ...
        for i in range(0, len(data) - config.ctx_len, config.batch_size):
            ...
            loss, grads = nn.value_and_grad(model, lambda m: nn.losses.cross_entropy(m.forward(x), y))(model)
